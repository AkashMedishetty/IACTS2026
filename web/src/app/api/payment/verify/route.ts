...
import { conferenceConfig, isEarlyBirdFaculty } from '@/config/conference.config'
import { memberCategoryKey, tierLabel, getCategoryAmount } from '@/lib/registration'
import crypto from 'crypto'
import Razorpay from 'razorpay'
import mongoose from 'mongoose'
import paymentAttempts from '@/lib/payment/attempts'
import { logPaymentError } from '@/lib/errors/service'
import { logPaymentAction } from '@/lib/audit/service'
...
async function recalculatePaymentBreakdown(user: any, totalAmount: number, currency: string) {
  try {
    ...
    let currentTierKey = 'regular'

    // Use getCurrentTier() as source of truth for tier selection
    const { getCurrentTier: getVerifyTier } = await import('@/lib/registration')
    const activeTierName = getVerifyTier()
    currentTierKey = activeTierName === 'Early Bird' ? 'earlyBird' : activeTierName === 'Spot Registration' ? 'onsite' : 'regular'
    currentTierName = activeTierName

    // Get pricing amounts from DB for the determined tier
    const pricingTiersConfig = pricingConfigs.find(c => c.key === 'pricing_tiers')
    if (pricingTiersConfig) {
      ...
      if (selectedTier?.categories) {
        registrationCategories = selectedTier.categories
        console.log('📊 Active pricing tier:', currentTierName, '(from getCurrentTier)')
      }
      ...
      if (Object.keys(registrationCategories).length === 0 && tiers.regular?.categories) {
        ...
        currentTierKey = 'regular'
        console.log('📊 Using regular tier as fallback')
      }
    }

    // Second pass: get accompanying person fee using the determined tier
    const accConfig = pricingConfigs.find(c => c.key === 'accompanying_person')
    if (accConfig?.value?.tierPricing) {
      const fee = accConfig.value.tierPricing[currentTierKey]
      if (fee !== undefined) {
        accompanyingPersonFeeFromDB = fee
      } else {
        accompanyingPersonFeeFromDB = accConfig.value.basePrice || 0
      }
      console.log('📊 Accompanying person tier lookup: tierKey=', currentTierKey, 'fee=', accompanyingPersonFeeFromDB)
    } else if (accConfig?.value?.basePrice) {
      accompanyingPersonFeeFromDB = accConfig.value.basePrice
    }
    ...
    const pricingType = isFaculty ? memberCategoryKey() : registrationType
    const registrationCategory = registrationCategories[pricingType]
    let baseAmount = registrationCategory?.amount || 0

    // Early Bird override for listed faculty (by phone) — use the Early Bird member rate
    const earlyBirdFaculty = isFaculty && isEarlyBirdFaculty(user.profile?.phone)
    if (earlyBirdFaculty) {
      const ebCfg = pricingConfigs.find(c => c.key === 'pricing_tiers')?.value
      const cfgAmt = ebCfg?.earlyBird?.categories?.[memberCategoryKey()]?.amount
      if (typeof cfgAmt === 'number' && cfgAmt > 0) {
        baseAmount = cfgAmt
      } else {
        try {
          baseAmount = getCategoryAmount(memberCategoryKey(), tierLabel('earlyBird'))
        } catch { baseAmount = 0 }
      }
    }
    ...
    const registrationTypeLabel = earlyBirdFaculty ? 'Faculty (Early Bird)' : isFaculty ? 'Faculty' : (registrationCategoryFromConfig?.label || registrationCategory?.label || registrationType)
    ...
    // Calculate GST
    // Faculty prices are GST-inclusive → extract the GST component from the total (do not add on top).
    // All other types: GST is 18% on top of the fees.
    const preGstTotal = baseAmount + totalWorkshopFees + accompanyingPersonFees + accommodationFees
    const gstAmount = isFaculty
      ? Math.round(preGstTotal - (preGstTotal / 1.18))
      : calculateGST(preGstTotal)

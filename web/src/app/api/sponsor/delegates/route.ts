...
import { logSponsorAction } from '@/lib/audit/service'
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (existingUser) {
      ...
      if (existingUser.registration?.status === 'pending-payment') {
        ...
        await sendEmailWithHistory({
          ...
          html: `<p>Dear ${existingUser.profile?.firstName},</p><p>Your registration for ${conferenceConfig.shortName} has been confirmed.</p><p>Registration ID: ${existingUser.registration.registrationId}</p>`,
          text: `Your registration for ${conferenceConfig.shortName} has been confirmed. Registration ID: ${existingUser.registration.registrationId}`,
    ...
    await sendEmailWithHistory({
      to: email,
      subject: `Welcome to ${conferenceConfig.shortName} - Registration Confirmed`,
      html: `
        <p>Dear ${title || 'Dr.'} ${firstName} ${lastName},</p>
        <p>Your registration for ${conferenceConfig.shortName} has been confirmed.</p>
        <hr/>
        ...
        <ul>
          <li><strong>Event:</strong> ${conferenceConfig.shortName}</li>
          <li><strong>Date:</strong> July 18-19, 2026</li>
          <li><strong>Venue:</strong> Hotel Daspalla, Hyderabad</li>
        </ul>
        <p>We look forward to seeing you at the conference!</p>
        <p>Best regards,<br/>${conferenceConfig.shortName} Team</p>
      `,
      text: `Your registration for ${conferenceConfig.shortName} has been confirmed. Registration ID: ${registrationId}. Login with your email and mobile number as password.`,
      userId: delegate._id,

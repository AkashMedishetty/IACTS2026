  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        ...
        if (existingUser) {
          ...
          if (existingUser.registration?.status === 'pending-payment') {
            ...
            await sendEmailWithHistory({
              to: email,
              subject: `${conferenceConfig.shortName} - Registration Confirmed by Sponsor`,
        ...
        await sendEmailWithHistory({
          to: email,
          subject: `Welcome to ${conferenceConfig.shortName} - Sponsored Registration`,
          html: `
            <p>Dear ${row.title || 'Dr.'} ${row.firstname} ${row.lastname},</p>
            <p>You have been registered for ${conferenceConfig.shortName} by <strong>${sponsor.sponsorProfile.companyName}</strong>.</p>
            <hr/>
          ...
          `,
          text: `You have been registered for ${conferenceConfig.shortName} by ${sponsor.sponsorProfile.companyName}. Registration ID: ${registrationId}. Login with your email and mobile number as password.`,
          userId: delegate._id,

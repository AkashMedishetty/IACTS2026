...
import ReviewerConfig from '@/lib/models/ReviewerConfig'
import { EmailService } from '@/lib/email/service'
import { logAction } from '@/lib/audit/service'
import { conferenceConfig } from '@/config/conference.config'

// Default email templates (fallback if not configured in ReviewerConfig)
const DEFAULT_ACCEPTANCE_SUBJECT = `Congratulations! Your Abstract {abstractId} Has Been Accepted - ${conferenceConfig.shortName}`
...
const DEFAULT_REJECTION_SUBJECT = `Update on Your Abstract Submission {abstractId} - ${conferenceConfig.shortName}`
  ...
  try {
    ...
    for (const pending of pendingEmails) {
      try {
        ...
        const dashboardUrl = `${process.env.APP_URL || process.env.NEXTAUTH_URL}/abstracts`

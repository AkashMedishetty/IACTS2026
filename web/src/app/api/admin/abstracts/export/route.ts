...
const submissionCategoryLabel = (k?: string) => (k ? (SUBMISSION_CATEGORY_LABELS[k] || k) : 'N/A')

function createCsv(abstracts: any[]) {
  const headers = [
    'Abstract ID', 'Registration ID', 'Name', 'Email', 'Mobile', 'Title',
    'Submission Category', 'Presenter Category', 'Track',
    'Submitting For', 'Topic', 'Authors', 'Status',
  ...
  const rows = abstracts.map(a => {
    ...
    const mobile = user?.profile?.phone || user?.phone || 'N/A'
    ...
    return [
      ...
      `"${mobile}"`,
      `"${(a.title || '').replace(/"/g, '""')}"`,
      submissionCategoryLabel(a.submissionCategory),
      `"${a.category || 'N/A'}"`,
      `"${a.track || 'N/A'}"`,
      a.submittingFor || 'N/A',

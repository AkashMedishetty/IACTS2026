...
import { conferenceConfig, getPlaceholderEmailDomain } from '@/config/conference.config'
...
export class EmailService {
    ...
    try {
      ...
      try {
        ...
        qrCodeBuffer = await QRCodeGenerator.generateRegistrationQRBuffer({
          ...
          name: paymentData.name,
          email: paymentData.email,
          type: paymentData.breakdown?.registrationType || ''
        })
        console.log('QR code generated for payment confirmation:', qrCodeBuffer ? `${qrCodeBuffer.length} bytes` : 'failed')
      } catch (qrError) {
        console.error('Failed to generate QR code for payment confirmation:', qrError)
      }
      ...
      if (qrCodeBuffer) {
        attachments.push({
          filename: `${paymentData.registrationId}-QR.png`,
  ...
  /**
   * Re-send a registrant their confirmation details: registration ID, QR code,
   * brochure link, event details (+ WhatsApp group via base template).
   * Used by the admin bulk "resend to all" tool.
   */
  static async sendRegistrationDetailsWithQR(data: {
    userId?: string
    email: string
    name: string
    registrationId: string
    ...
    try {
      const site = process.env.APP_URL || process.env.NEXT_PUBLIC_APP_URL || conferenceConfig.contact.website || ''
      const eventStart = new Date(conferenceConfig.eventDate.start).toLocaleDateString('en-US', { month: 'long', day: 'numeric' })
      const eventEnd = new Date(conferenceConfig.eventDate.end).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })
      ...
      try {
        ...
        console.error('QR generation failed for resend:', qrError)
      }

      const content = `
        <h2>Your ${conferenceConfig.shortName} Registration</h2>
        <p>Dear ${data.name},</p>
        <p>Here are your registration details for <strong>${conferenceConfig.name}</strong>. Please keep this email handy — your QR code below is your entry pass at the registration desk.</p>

        <div class="highlight">
          <h3>Registration Details</h3>
          <table>
            <tr><th>Registration ID</th><td><strong>${data.registrationId}</strong></td></tr>
            ${data.registrationType ? `<tr><th>Category</th><td>${data.registrationType}</td></tr>` : ''}
            <tr><th>Email</th><td>${data.email}</td></tr>
            <tr><th>Dates</th><td>${eventStart} – ${eventEnd}</td></tr>
        ...
        <p>Best regards,<br><strong>${conferenceConfig.shortName} Organising Committee</strong></p>
      `

      const attachments: any[] = []
      if (qrCodeBuffer) {
        attachments.push({ filename: `${data.registrationId}-QR.png`, content: qrCodeBuffer, contentType: 'image/png' })
        attachments.push({ filename: 'qr-code-embedded.png', content: qrCodeBuffer, contentType: 'image/png', cid: 'qr-code-embedded' })
      }
      ...
      return await sendEmail({
        ...
        userId: data.userId,
        userName: data.name,
        templateName: 'registration-details-resend',
        category: 'registration'
      })
    } catch (error) {
      console.error('Error sending registration details:', error)
      return { success: false, error: error instanceof Error ? error.message : 'Unknown error' }
    }
  }
      ...
      const html = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <ul style="color: #374151; font-size: 16px; line-height: 1.6;">
            ...
            <li>If accepted, you'll be invited to submit your final presentation on the abstracts page</li>
            <li>You can track your submission status on our website</li>
          </ul>
          
          <div style="text-align: center; margin: 30px 0;">
            <a href="${process.env.APP_URL}/abstracts" 
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <div style="background-color: #fef3c7; border: 1px solid #fbbf24; border-radius: 8px; padding: 15px; margin: 25px 0;">
            <p style="margin: 0; color: #92400e; font-size: 14px;">
              ...
              • Final Presentation Deadline: [To be announced]<br>
              • Conference Dates: ${conferenceConfig.eventDate.start} to ${conferenceConfig.eventDate.end}<br>
        ...
        case 'badge':
          ...
          try {
            ...
            if (emailData.userData.registrationId) {
              ...
              qrCodeCID = `qrcode-${emailData.userData.registrationId}@${getPlaceholderEmailDomain()}`

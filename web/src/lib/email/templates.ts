    ...
    <head>
      ...
      <style>
        ...
        .header {
          ...
          margin-bottom: 30px;
          padding-bottom: 20px;
          border-bottom: 3px solid #A56C00;
        }
        .logo {
          font-size: 28px;
          font-weight: bold;
      ...
      <div class="container">
        ...
        <div class="footer">
          ...
          <p style="margin-top: 8px; color: #4b5563;">
            Powered by <a href="https://purplehatevents.in" style="color: #A56C00; text-decoration: none; font-weight: 600;">PurpleHat Events</a>
          </p>
        </div>
      </div>
    </body>
...
export function getPaymentConfirmationTemplate(paymentData: {
  ...
  const currencySymbol = paymentData.currency === 'INR' ? '₹' : paymentData.currency
  const baseAmount = paymentData.breakdown.baseAmount || paymentData.breakdown.registration || 0
  const gst = paymentData.breakdown.gst || 0
  const gstPct = paymentData.breakdown.gstPercentage || 18
  const workshopTotal = paymentData.breakdown.workshops || 0
  const accompanyingTotal = paymentData.breakdown.accompanyingPersonFees || paymentData.breakdown.accompanyingPersons || 0
  const accompanyingCount = paymentData.breakdown.accompanyingPersonCount || 0
  const discount = paymentData.breakdown.discount || 0
  const regLabel = paymentData.breakdown.registrationTypeLabel || paymentData.breakdown.registrationType || ''
  ...
  const content = `
    ...
    <div class="highlight">
      ...
      <table>
        ...
        <tr><th>Amount Paid</th><td><strong>${currencySymbol}${paymentData.amount.toLocaleString('en-IN')}</strong></td></tr>
        <tr><th>Payment Date</th><td>${paymentData.paymentDate}</td></tr>
        <tr><th>Payment Method</th><td>${paymentData.breakdown.paymentMethod === 'payment_gateway' ? 'Online Payment (Razorpay)' : paymentData.breakdown.paymentMethod === 'bank-transfer' ? 'Bank Transfer' : 'Online Payment'}</td></tr>
    ...
    <div style="text-align: center; margin: 24px 0; padding: 20px;">
      <div style="display: inline-block; padding: 16px; background: #ffffff; border: 2px solid #A56C00; border-radius: 10px;">
        <img src="cid:qr-code-embedded" alt="Registration QR Code" style="width: 180px; height: 180px; display: block;" />
        <p style="margin: 10px 0 0 0; font-size: 14px; font-weight: bold; color: #A56C00; font-family: 'Courier New', monospace;">${paymentData.registrationId}</p>
      </div>
    ...
    <div class="highlight">
      ...
      <table style="width: 100%; border-collapse: collapse;">
        <tr style="background: #f8f9fa;">
          <th style="width: 60%; text-align: left; padding: 8px; border-bottom: 1px solid #dee2e6;">Description</th>
          <th style="width: 40%; text-align: right; padding: 8px; border-bottom: 1px solid #dee2e6;">Amount</th>
        </tr>
        <tr>
          <td style="padding: 8px; border-bottom: 1px solid #eee;">Registration Fee (${regLabel})</td>
          <td style="text-align: right; padding: 8px; border-bottom: 1px solid #eee;">${currencySymbol}${baseAmount.toLocaleString('en-IN')}</td>
        </tr>
        ${paymentData.breakdown.workshopFees && paymentData.breakdown.workshopFees.length > 0 ? 
          paymentData.breakdown.workshopFees.map((w: any) => 
            `<tr><td style="padding: 8px; border-bottom: 1px solid #eee;">Workshop: ${w.name}</td><td style="text-align: right; padding: 8px; border-bottom: 1px solid #eee;">${currencySymbol}${w.amount.toLocaleString('en-IN')}</td></tr>`
          ).join('') : ''}
        ${accompanyingCount > 0 ? 
          `<tr><td style="padding: 8px; border-bottom: 1px solid #eee;">Accompanying Person${accompanyingCount > 1 ? 's' : ''} (${accompanyingCount})</td><td style="text-align: right; padding: 8px; border-bottom: 1px solid #eee;">${currencySymbol}${accompanyingTotal.toLocaleString('en-IN')}</td></tr>
          ${paymentData.breakdown.accompanyingPersonDetails ? 
            paymentData.breakdown.accompanyingPersonDetails.map((p: any) => 
              `<tr><td colspan="2" style="font-size: 12px; color: #666; padding: 4px 8px 4px 24px;">↳ ${p.name} (Age: ${p.age})</td></tr>`
            ).join('') : ''}` : ''}
        ${discount > 0 ? 
          `<tr><td style="padding: 8px; border-bottom: 1px solid #eee; color: #16a34a;">Discount</td><td style="text-align: right; padding: 8px; border-bottom: 1px solid #eee; color: #16a34a;">-${currencySymbol}${discount.toLocaleString('en-IN')}</td></tr>` : ''}
        <tr>
          <td style="padding: 8px; border-bottom: 1px solid #eee;">Subtotal</td>
          <td style="text-align: right; padding: 8px; border-bottom: 1px solid #eee;">${currencySymbol}${(baseAmount + workshopTotal + accompanyingTotal - discount).toLocaleString('en-IN')}</td>
        </tr>
        <tr>
          <td style="padding: 8px; border-bottom: 1px solid #dee2e6;">GST (${gstPct}%)</td>
          <td style="text-align: right; padding: 8px; border-bottom: 1px solid #dee2e6;">${currencySymbol}${gst.toLocaleString('en-IN')}</td>
        </tr>
        <tr style="background: #A56C00; color: white; font-weight: bold; font-size: 16px;">
          <td style="padding: 10px 8px;">TOTAL PAID</td>
          <td style="text-align: right; padding: 10px 8px;">${currencySymbol}${paymentData.amount.toLocaleString('en-IN')}</td>
        </tr>
      </table>
    </div>
    ...
    <ul>
      ...
      <li>Your QR code is attached as a PNG file — save it on your phone for quick check-in</li>
    ...
    <p style="text-align: center;">
      <a href="${process.env.APP_URL || conferenceConfig.contact.website}/dashboard" class="button">View Your Dashboard</a>
    </p>
...
export function getRegistrationAcceptanceTemplate(userData: {
  ...
  const content = `
    ...
    <div style="text-align: center; margin: 20px 0; padding: 20px;">
      <div style="display: inline-block; padding: 16px; background: #ffffff; border: 2px solid #A56C00; border-radius: 10px;">
        <img src="cid:qr-code-embedded" alt="Registration QR Code" style="width: 180px; height: 180px; display: block;" onerror="this.src='${userData.qrCodeDataURL}'" />
        <p style="margin: 10px 0 0 0; font-size: 14px; font-weight: bold; color: #A56C00; font-family: 'Courier New', monospace;">${userData.registrationId}</p>
      </div>
      <p style="margin: 8px 0 0 0; font-size: 12px; color: #6b7280;">Present this QR at the registration desk</p>
    </div>
    ` : ''}
    ...
    <div style="background-color: #fffdfc; padding: 15px; border-left: 4px solid #A56C00; margin: 20px 0;">
      ...
      <ul style="margin-bottom: 0;">
        ...
        <li>Your invoice receipt is attached to this email</li>
        ${userData.brochureUrl ? `<li>You can download the conference brochure/programme <a href="${userData.brochureUrl}" target="_blank" style="color: #A56C00; font-weight: bold; text-decoration: underline;">here</a></li>` : '<li>Conference program and schedule will be shared 2 weeks before the event</li>'}
    ...
    <p style="text-align: center; margin: 25px 0;">
      <a href="${process.env.APP_URL || conferenceConfig.contact.website}/dashboard" class="button">Access Your Dashboard</a>
    </p>
    
    <p><strong>Important:</strong> Please save this email and the attached files (QR Code, invoice, and badge if attached). Bring them on your mobile or as a printout for entry.</p>
...
export function getEventReminderTemplate(userData: {
  ...
  const content = `
    <h2>⏰ Reminder: ${conferenceConfig.shortName} is Coming Up!</h2>
    <p>Dear ${userData.name},</p>
    ...
    <div style="background-color: #fff3cd; padding: 15px; border-left: 4px solid #ffc107; margin: 20px 0;">
      ...
      <ul style="margin-bottom: 0;">
        <li>✓ Check-in opens at 8:00 AM</li>
        <li>✓ Bring your printed badge or mobile device</li>
        <li>✓ Review the conference schedule on our website</li>
        <li>✓ Prepare your presentation (if applicable)</li>
      </ul>
    </div>

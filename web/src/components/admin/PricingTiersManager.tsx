...
export function PricingTiersManager() {
  ...
  // Dynamically extract category keys and labels from loaded pricing data
  const getCategoryKeys = () => {
    if (!pricingData || !pricingData.earlyBird) return []
    return Object.keys(pricingData.earlyBird.categories)
  }
  
  const getCategoryLabel = (key: string) => {
    if (!pricingData || !pricingData.earlyBird) return key
    return pricingData.earlyBird.categories[key]?.label || key
  }
        ...
        {getCategoryKeys().map(key => (
          <div key={key} className="grid grid-cols-1 md:grid-cols-3 gap-3 p-4 border rounded-lg">
            <div className="md:col-span-1">
              <Label className="text-sm font-medium">{getCategoryLabel(key)}</Label>
            </div>
            <div>
...
function NewSpecialOfferForm({ 
  ...
  // Load category structure from conference config dynamically
  const [pricingData, setPricingData] = useState<any>(null)
  const [loading, setLoading] = useState(true)
  
  useEffect(() => {
    fetch('/api/admin/config/pricing-tiers')
      .then(res => res.json())
      .then(result => {
        if (result.success && result.data?.earlyBird) {
          setPricingData(result.data)
          // Initialize formData categories from loaded data
          const initialCategories: any = {}
          Object.keys(result.data.earlyBird.categories).forEach(key => {
            initialCategories[key] = {
              amount: 5000,
              currency: 'INR',
              label: result.data.earlyBird.categories[key].label
            }
          })
          setFormData(prev => ({
            ...prev,
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {Object.keys(formData.categories).map(key => (
              <div key={key} className="grid grid-cols-1 md:grid-cols-3 gap-3 p-4 border rounded-lg">
                <div className="md:col-span-1">
                  <Label className="text-sm font-medium">{formData.categories[key]?.label || key}</Label>
                </div>
                <div>
                  <Label className="text-xs text-muted-foreground">Amount</Label>
                  <Input

            ...
            <TabsContent value="settings" className="space-y-4">
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label className="text-slate-900 dark:text-white">Canvas Width (px)</Label>
                    <Input
                      type="number"
                      value={templateDimensions.width}
                      onChange={(e) => setTemplateDimensions(prev => ({ ...prev, width: Math.max(100, parseInt(e.target.value) || 0) }))}
                      className="mt-2"
                      min={100}
                    />
                  </div>
                  <div>
                    <Label className="text-slate-900 dark:text-white">Canvas Height (px)</Label>
                    <Input
                      type="number"
                      value={templateDimensions.height}
                      onChange={(e) => setTemplateDimensions(prev => ({ ...prev, height: Math.max(100, parseInt(e.target.value) || 0) }))}
                      className="mt-2"
                      min={100}
                    />
                  </div>
